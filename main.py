...
def filtered_transactions_by_state(transactions: list[dict], suggest_choose_state: str) -> list[dict]:
    ...
    state = user_input_validation(['EXECUTED', 'CANCELED', 'PENDING', ''], 'Введите один из трех статусов или пустую строку: ').upper()
    if state.lower() == 'exit':
...
def main():
    ...
    for transaction in transactions:
        ...
        account_from = masked_account_card(transaction['from'])
        account_to = masked_account_card(transaction['to'])
        arrow = ' -> '
